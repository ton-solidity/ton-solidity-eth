...
void CSECodeGenerator::generateClassElement(Id _c, bool _allowSequenced)
{
	...
	if (expr.item->type() != Operation || instructionInfo(expr.item->instruction(), VMMachineAndVersion()).ret == 1)
	{
	...
	{
		assertThrow(
			instructionInfo(expr.item->instruction(), VMMachineAndVersion()).ret == 0,
			OptimizerException,
